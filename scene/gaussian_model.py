    ...
    def update_learning_rate(self, iteration,args):
        ...
        for param_group in self.optimizer.param_groups:
            if param_group["name"] == "xyz":
                ...
                if not (hasattr(args, 'enable_heartbeat_albedo') and args.enable_heartbeat_albedo):
                    param_group['lr'] = xyz_lr 
                else:
                    param_group['lr'] = 0.0  # freeze xyz during rppg training

            # scheduler for albedo modulation parameters
            elif param_group["name"] == "heartbeat_phase" and hasattr(self, 'heartbeat_phase_scheduler'):
                param_group['lr'] = self.heartbeat_phase_scheduler(iteration)
            elif param_group["name"] == "heartbeat_raw_deltas" and hasattr(self, 'heartbeat_raw_delta_scheduler'):
                param_group['lr'] = self.heartbeat_raw_delta_scheduler(iteration)
            elif param_group["name"] == "heartbeat_mlp":
                param_group['lr'] = self.heartbeat_mlp_scheduler(iteration)
